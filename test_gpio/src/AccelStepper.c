...
uint8_t runSpeed(Stepper_t* motor)
{
	...
	unsigned long time = HAL_GetTick(); //Arduino: micros();
...
void computeNewSpeed(Stepper_t* motor)
{
    ...
    if (distanceTo > 0)
    {
		...
		if (motor->_direction == DIRECTION_CCW) motor->_speed = -motor->_speed;

#if ACCEL_STEPPER_DEBUG
...
void step1(Stepper_t* motor, long step)
{
    ...
    HAL_Delay(motor->_minPulseWidth); //delayMicroseconds(motor->_minPulseWidth);
...
{
	motor->_pinInverted[0] = stepInvert;
	motor->_pinInverted[1] = directionInvert;
